...
def find_analogues_onetime(field_gcm, obs_coarse, time_mapper, n_analogues = 30,
                           window_unit = 'days', metric = 'RMSE', transform = None, box_length = 10):
    """
    ...
    * transform ({'sqrt'}) - The name of a function to be used to transform field_gcm and obs_coarse before selecting
    ...
    lon_max = center_lon + half_box

    # Subset obs_coarse to smaller domain
    obs_coarse_smaller = obs_coarse.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))
    # Also, subset field_gcm to match the smaller domain
    field_gcm_smaller = field_gcm.sel(lat=slice(lat_min, lat_max), lon=slice(lon_min, lon_max))

    # select observations within a similar part of the year to the simulated time
    gcm_time = field_gcm.time
    obs_candidates = get_obs_candidates(obs_coarse_smaller, gcm_time, time_mapper, window_unit)

    # transform the fields if necessary
    if transform == 'sqrt':
        field_gcm_smaller = np.sqrt(field_gcm_smaller)
        obs_coarse_smaller = np.sqrt(obs_coarse_smaller)
    ...
    measures = metric_func(field_gcm_smaller, obs_candidates, dim = ['lat', 'lon'])
...
def construct_analogue_onetime(field_gcm, obs_coarse, obs_fine, time_mapper,
                               ...
                               penalty = 'l2', jitter = False, jitter_thresh = '0.1 mm/d', transform = None, box_length = 10 ):
    """
    ...
    * transform ({'sqrt'}) - The name of a function to be used to transform field_gcm and obs_coarse before selecting
    ...
    obs_analogues = find_analogues_onetime(field_gcm, obs_coarse, time_mapper,
                                            n_analogues = n_analogues, window_unit = window_unit,
                                            metric = metric, transform = transform, box_length = box_length)
...
def process_chunk(data_gcm_chunk, obs_coarse, obs_fine, time_mapper, metric, n_analogues, window_unit, jitter, jitter_thresh, transform, penalty, box_length):
    ...
    for i in range(len(data_gcm_chunk.time)):
        CA_sample = construct_analogue_onetime(data_gcm_chunk.isel(time=i),
                                               obs_coarse,
                                               obs_fine,
                                               time_mapper,
                                               metric=metric,
...
def construct_analogues(data_gcm, obs_coarse, obs_fine,
                        ...
                        penalty = 'l2', jitter = False, jitter_thresh = '0.1 mm/d', transform = None,
                        fout = None, write_output = False, box_length = 10):
    """
    ...
    * write_output (bool) - If True, write output to fout and return None. Otherwise, return the downscaled data.
    ...
    num_chunks = np.ceil(ntime / chunk_size).astype(int)

    concatenated_results = []
    for chunk_idx in tqdm(range(num_chunks), desc='Processing chunks'):
        start_idx = chunk_idx * chunk_size
        end_idx = min((chunk_idx + 1) * chunk_size, ntime)
        data_gcm_chunk = data_gcm.isel(time=slice(start_idx, end_idx))

        # Process the current chunk
        da_list_chunk = process_chunk(data_gcm_chunk, obs_coarse, obs_fine, time_mapper, metric,
                                      n_analogues, window_unit, jitter, jitter_thresh, transform, penalty, box_length)

        # Compute the results for the current chunk
        da_list_comp_chunk = dask.compute(*da_list_chunk)
        #print("data_gcm_chunk-------------------\n", data_gcm_chunk)
        #print("da_list_comp_chunk-------------------", da_list_comp_chunk)
        # Assuming you have a list of computed daily results for a year and their corresponding times

        corresponding_times = data_gcm_chunk.time  # Datetime objects representing each day in daily_results_for_year

        # Ensure each result has a 'time' dimension
        for i, result in enumerate(da_list_comp_chunk):
            # Convert the datetime object to a DataArray with the same shape as 'result' but with 'time' as its dimension
            time_da = xr.DataArray([corresponding_times[i]], dims=["time"])
            # Assign this 'time' DataArray to your result. If 'result' doesn't already have a 'time' dimension, this will add it.

            result_with_time = result.expand_dims({'time':time_da})
            #print("result_with_time-------------------\n", result_with_time)
            # Now append this to your list for concatenation
            concatenated_results.append(result_with_time)

        ##concatenated_results.extend(da_list_comp_chunk)
        #for da in concatenated_results:
        #        print("da.dims--------\n", da.dims)
        #        print("da.coords--------\n",da.coords)

        # Clear memory
        del data_gcm_chunk, da_list_chunk, da_list_comp_chunk, corresponding_times
        gc.collect()

    # Concatenate all processed data
    data_CA = xr.concat(concatenated_results, dim='time')
    ## Assuming concatenated_results is a list of DataArrays
    #for da in concatenated_results:
    #    print(da.dims, "da.dims--------\n")
    #    print(da.coords,"da.coords--------\n")
#
    ## Verify the length of the time coordinate in each DataArray
    #for da in concatenated_results:
    #    print(len(da.coords['time']),"len(da.coords['time'])--------\n")
#
    ## Assuming data_gcm has the correct time dimension you want to preserve
    #print(len(data_gcm.coords['time']),"len(data_gcm.coords['time'])--------\n")
...
def BCCA(data_gcm_hist, data_gcm_future, data_obs_fine,
         ...
         write_output = False, fout_hist = None, fout_future = None, do_future = True, box_length = 10):
    """
    ...
    * do_future (bool) - If True, downscale both data_gcm_hist and data_gcm_future. Otherwise, only do BCCA on data_gcm_hist.
    ...
    """
    #print('0 hist data_gcm_hist =====================================================\n', data_gcm_hist.isel(time = 0).values)
    ...
    #print('1 hist data_gcm_hist =====================================================\n', data_gcm_hist.isel(time = 10).values)
    ...
    data_gcm_hist_CA = construct_analogues(data_gcm_hist_bc, data_obs_coarse, data_obs_fine,
                                            ...
                                            fout = fout_hist, box_length = box_length)
    if do_future:
        ...
        data_gcm_future_CA = construct_analogues(data_gcm_future_bc, data_obs_coarse, data_obs_fine,
                                                 ...
                                                 transform = transform,
                                                 write_output = write_output,
                                                 fout = fout_future, box_length = box_length)
    else:
